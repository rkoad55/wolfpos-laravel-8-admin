...
@endif
              ...
              <div class="card-tools">
                ...
                <a href="{{ url('/category/create') }}" class="fcbtn btn btn-outline btn-primary btn-1b">Add Category</a>
									...
									<table id="example-5" class="display mb-4 dataTablesCard">
										<thead>
                                            <tr>
                                                ...
                                                <th><strong class="font-w600 wspace-no">User Name & Category</strong></th>
...
<tr id="row_{{$value->id}}">
  ...
  <td>{{$value->name}}</td>
  <td>@if($value->user != null) {{ $value->user->name}}  @endif @if($value->categories != null) , {{  $value->categories->name}}  @endif</td>
  ...
  <div class="dropdown ml-auto">
		...
		<form  method="POST" action="{{ url('/categoriesExtra/show') }}">
                      						@csrf
                                              <input type="hidden" name="categoriesExtra_id" value="{{$value->id}}"/>
			...
			<a class="dropdown-item text-black" href="{{ url('/category/'.$value->id.'/edit') }}">
			<svg class="mr-3" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
